from datetime import datetime, timedelta, time
from openpyxl import Workbook, load_workbook
import selenium.common.exceptions
from selenium import webdriver
from selenium.webdriver.common.action_chains import ActionChains
from selenium.webdriver.support import expected_conditions as ec
from selenium.webdriver.support.ui import WebDriverWait
from selenium.webdriver.common.by import By
from selenium.webdriver.common.keys import Keys
from time import sleep
from threading import Thread
from bs4 import BeautifulSoup
import speech_recognition as sr
import lxml
import pytimedinput
import pyaudio


class Schedule:

    def __init__(self, code, entry_time, exit_time, driver):
        print(f">new_class created:[{datetime.now()}] code :: {code} :: entry_time :: {entry_time} :: exit_time :: {exit_time} ")
        self.code = code
        self.entry_time = entry_time
        self.exit_time = exit_time
        self.link = self.links[code]
        self.driver = driver

        self.exited = False
        self.attendee_list_open = False
        self.joined_meet = False
        self.chat_open = False
        self.processing_attendance = False

        if self.link is None:
            self.link = input(f"Please enter the link for {self.code} :: ")
        self.link = f"'{self.link}'"
        self.join_meet()


    links = {
        "AP": None,
        "EE": "https://meet.google.com/dju-rikc-dqp",
        "MA": "https://meet.google.com/lookup/egu4culb4h?authuser=0&hs=179",
        "CO": "https://meet.google.com/lookup/eflwgh5lju?authuser=0&hs=179",
        "EELAB": None,
        "COLAB": "https://meet.google.com/lookup/fpslolbjbv?authuser=0&hs=179",
        "APLAB": "https://meet.google.com/fha-actr-rqk",
        "MELAB": "https://meet.google.com/lookup/ay6bqb34ol?authuser=0&hs=179",
        "BCOM": 'https://meet.google.com/tom-cwzr-tce',
        "TESTING": 'https://meet.google.com/fnm-kfkm-yir'
    }

    xpath = {
        "show_people": '//*[@id="ow3"]/div[1]/div/div[9]/div[3]/div[10]/div[3]/div[2]/div/div/div[2]/span/button/i[1]',
        "close_people": '//*[@id="ow3"]/div[1]/div/div[9]/div[3]/div[4]/div[2]/div[1]/div[2]/div/span/button/i',
        "mic_main": '//*[@id="yDmH0d"]/c-wiz/div/div/div[9]/div[3]/div/div/div[4]/div/div/div[1]/div[1]/div/div[4]/div[1]/div/div/div/span/span',
        "camera_main": '//*[@id="yDmH0d"]/c-wiz/div/div/div[9]/div[3]/div/div/div[4]/div/div/div[1]/div[1]/div/div[4]/div[2]/div/div/span/span',
        # Xpath for changing background option, only available in the lobby
        "lobby_check": '//*[@id="yDmH0d"]/c-wiz/div/div/div[9]/div[3]/div/div/div[4]/div/div/div[1]/div[1]/div[1]/div[6]/div/span/span/span',
        "join_button": '//*[@id="yDmH0d"]/c-wiz/div/div/div[9]/div[3]/div/div/div[4]/div/div/div[2]/div/div[2]/div/div[1]/div[1]',

        "open_chat": '/html/body/div[1]/c-wiz/div[1]/div/div[9]/div[3]/div[10]/div[3]/div[2]/div/div/div[3]/span/button/i[1]',
        "chat_box": '//*[@id="ow3"]/div[1]/div/div[9]/div[3]/div[4]/div[3]/div[2]/div/div[4]/div[1]/div[1]/div[2]/textarea',
        "close_chat": '//*[@id="ow3"]/div[1]/div/div[9]/div[3]/div[4]/div[3]/div[1]/div[2]/div/span/button/i'
    }

    def join_meet(self):
        self.exited = False
        driver = self.driver
        try:
            driver.switch_to.window(driver.window_handles[0])
        except selenium.common.exceptions.InvalidSelectorException:
            print(">Invalid session exception")
            print(">Passing on")

        wait = WebDriverWait(driver, 60)
        driver.execute_script(f"window.open({self.link});")
        tabs = driver.window_handles
        current_tab = tabs[len(tabs) - 1]
        driver.switch_to.window(current_tab)
        sleep(2)
        mic_main = wait.until(ec.visibility_of_element_located((By.XPATH, self.xpath["mic_main"])))
        action_1 = ActionChains(driver)
        action_1.move_to_element(mic_main).click().perform()
        sleep(1)
        # 1 in 10 times it clicks but fails to turn off the camera!!
        camera_main = wait.until(ec.visibility_of_element_located((By.XPATH, self.xpath["camera_main"])))
        action_2 = ActionChains(driver)
        action_2.move_to_element(camera_main).click().perform()
        sleep(1)
        while True:
            try:
                join_button = wait.until(ec.visibility_of_element_located((By.XPATH, self.xpath["join_button"]))).click()
                break
            except selenium.common.exceptions.ElementClickInterceptedException:
                pass
        print(">Waiting in the lobby")
        wait_2 = WebDriverWait(driver=driver, timeout=100000)
        try:
            wait_2.until_not(ec.visibility_of_element_located((By.XPATH, self.xpath["lobby_check"])))
        except selenium.common.exceptions.TimeoutException:
            print(">Class entry denied")
            self.join_meet()
        print(">Class Joined")
        self.joined_meet = True

    def roll_number(self, encore):
        encore = encore.lower()
        encore = encore.split(" ")
        temp = ''
        for split in encore:
            if split.isalpha() is False:
                temp += split

        cross = ''
        for i in temp:
            if i.isnumeric():
                cross += i
            elif i == 'o':
                cross += i

        cross = cross.replace("17", '', 1)
        cross = cross.replace("22", '', 1)
        cross = cross.replace('o', '0')
        if len(cross) == 0:
            cross = '0'

        cross = int(cross)
        if cross == 7500:
            cross = 75
        return cross

    def attendance(self):
        self.processing_attendance = True
        print(">Attendance started")
        run_interval = 2.12
        driver = self.driver
        wait = WebDriverWait(driver, 60)
        attendance_file = f"{self.code}.xlsx"
        wb = load_workbook(attendance_file)
        ws = wb.active

        # Creating a list of members which already exist in the xl file
        attendee_names_xl = []
        attendees = ws.iter_cols(min_row=2, max_row=ws.max_row, min_col=1, max_col=1)
        # Even though a singular tuple is being returned in this case
        # The generator objects acts ina  way that multiple tuples are
        # being returned hence we must unpack the first tuple and then access it
        for tuple_1 in attendees:
            attendees = tuple_1

        # Checking for previously existing names in xl file
        for attendee in attendees:
            attendee_names_xl.append(attendee.value)

        # Finding the date column/ Creating one if it doesnt exist already
        max_col = ws.max_column
        dates = ws.iter_rows(min_row=1, max_row=1, min_col=2, max_col=max_col)
        for tuple_1 in dates:
            dates = tuple_1
        date_cell = None

        for date in dates:
            if date.value == self.entry_time:
                date_cell = date
                break

        if date_cell is None:
            date_cell = ws.cell(row=1, column=max_col+1, value=self.entry_time)
        main_col = date_cell.column

        print("")
        print(">", date_cell)
        print(">", date_cell.value)

        while self.exited is False:
            while self.chat_open is True:
                pass
            print(">Opening attendee list")

            if self.attendee_list_open is False:
                self.attendee_list_open = True
                action_3 = ActionChains(driver)
                show_people = wait.until(ec.visibility_of_element_located((By.XPATH, self.xpath["show_people"])))
                action_3.move_to_element(show_people).click().perform()

            elif self.attendee_list_open is True:
                pass

            start_check = datetime.now()
            sleep(5)
            print(f">[{datetime.today()}] Checking for attendees")
            names = []
            html_text = driver.page_source
            print(">Closing attendee list")

            try:
                close_people = wait.until(ec.element_to_be_clickable((By.XPATH, self.xpath["close_people"]))).click()
                self.attendee_list_open = False
            except selenium.common.exceptions.TimeoutException:
                print(">Failed to close attendance list!")
                self.attendee_list_open = True

            soup = BeautifulSoup(html_text, 'lxml')
            list_ = soup.find("div", {"role": "list"})
            items = list_.find_all("div", {"role": "listitem"})

            for item in items:
                text = item.text
                names.append(text)

            # adding new attendees to the xl file
            for name in names:
                if name not in attendee_names_xl:
                    new_attendee = ws.cell(row=ws.max_row + 1, column=1, value=name)
                    attendee_names_xl.append(name)

            # Refreshing the tuple containing cell objects after adding new cells in xl file
            attendees = ws.iter_cols(min_row=2, max_row=ws.max_row, min_col=1, max_col=1)
            for tuple_1 in attendees:
                attendees = tuple_1

            # Adding time for each attendee
            for attendee in attendees:
                if attendee.value in names:

                    time_add = ws.cell(row=attendee.row, column=main_col)
                    time_elapsed = datetime.now() - start_check
                    time_elapsed = time_elapsed.total_seconds()
                    time_elapsed = time_elapsed/60

                    if time_add.value is None:
                        time_add.value = run_interval + time_elapsed
                    else:
                        time_add.value += run_interval + time_elapsed
                else:
                    pass

            print(f">[{datetime.today()}] Completed checking for attendees")
            print(">Saving attendance")
            wb.save(f"{self.code}.xlsx")

            # So that the thread terminates once the class has ended even if its is sleeping for 'run_interval'
            now = datetime.today()
            while datetime.today() - now < timedelta(minutes=run_interval):
                if self.exited is True:
                    break
                else:
                    pass

        # Assigning roll numbers to meet members
        for cell in range(2, ws.max_row + 1):
            encore = ws['A' + str(cell)].value
            ws['B' + str(cell)].value = self.roll_number(encore)
        print(">Saving attendance before closing the file")
        wb.save(f"{self.code}.xlsx")
        wb.close()
        self.processing_attendance = False

    def exit_meeting(self):
        self.exited = True
        while self.processing_attendance is True:
            pass
        self.driver.close()
        print(">Exited class")

    def speech_recognition(self):
        print(">Speech Recognition started")
        recog = sr.Recognizer()

        while self.exited is False:
            with sr.Microphone() as source:
                audio = recog.listen(source)
                try:
                    text = recog.recognize_google(audio, language='en-IN')
                    print(f"[speech]  {text}")
                except:
                    print("[speech] Failed to recognize")

    def chat_type(self, text):

        driver = self.driver
        wait = WebDriverWait(driver, 12000000)
        while self.attendee_list_open is True:
            pass
        sleep(1)
        self.chat_open = True
        open_chat = wait.until(ec.element_to_be_clickable((By.XPATH, self.xpath["open_chat"]))).click()
        sleep(1)
        chat_box = wait.until(ec.visibility_of_element_located((By.XPATH, self.xpath["chat_box"])))
        type = ActionChains(driver)
        type.click(chat_box).pause(1).send_keys_to_element(chat_box, text).perform()
        print(f">Entering into the chat box {text}")
        chat_box.send_keys(Keys.ENTER)
        open_chat = wait.until(ec.element_to_be_clickable((By.XPATH, self.xpath["close_chat"]))).click()
        sleep(1)
        self.chat_open = False

    def create_threads(self):

        while self.joined_meet is False:
            pass
        t1 = Thread(target=self.attendance)
        # t2 = Thread(target=self.speech_recognition)
        return [t1]

    def __del__(self):
        print(f">Class object killed: [{datetime.now()}] id :: {self.code} :: entry_time :: {self.entry_time} :: exit_time :: {self.exit_time}")


# exit automatically {auto_exit_after} min's later post exit_time
auto_exit_after = 2
data_file = "time_table.xlsx"
wb = load_workbook(data_file)
ws = wb.active
class_is = False


def start_threads(threads):
    print(">Starting Threads")
    for thread in threads:
        thread.start()
    print(">All threads started")


def join_threads(threads):
    print(">Joining threads")
    for thread in threads:
        if thread.is_alive():
            thread.join()
    print(">All threads joined")


def max_row(workseet):
    # ws.max_row was returning false value
    i = 1
    while workseet.cell(row=i, column=1).value is not None:
        i += 1
    return i-1


eod_row = max_row(ws)
EOD = ws.cell(row=eod_row, column=1).value


def open_chrome(profile):
    options = webdriver.ChromeOptions()
    options.add_argument("--user-data-dir=C:/Users/defn0t/AppData/Local/Google/Chrome/User Data")
    options.add_argument(f'--profile-directory={profile}')
    # options.add_argument('--disable-blink-features=AutomationControlled')
    options.add_experimental_option("prefs", {
        "profile.default_content_setting_values.media_stream_mic": 1,
        "profile.default_content_setting_values.media_stream_camera": 1,
        "profile.default_content_setting_values.geolocation": 2,
        "profile.default_content_setting_values.notifications": 1
    })
    driver = webdriver.Chrome(options=options, executable_path='C:\webdrivers\chromedriver.exe')
    driver.get("https://google.com")
    return driver


def time_check():
    # Will Return a class if one lies in current time
    # Will Return None if EOD
    # Will pause till next class if no class in current time & its not EOD

    today = datetime.today()
    if today.time() > EOD:
        return None

    row = None
    column = None
    code = None
    week_day = today.strftime("%a")
    cols = ('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H')

    for col in cols:
        cell = col + "1"
        if week_day == ws[cell].value:
            column = col
            break

    for i in range(2, eod_row):
        cell = "A" + str(i)
        cell_ahead = "A" + str(i + 1)
        if ws[cell].value < today.time() < ws[cell_ahead].value:
            row = i
            break

    # staring below the first class row will come out as None
    if row is not None:
        code = ws[column + str(row)].value
    else:
        row = 1
    # The following will give us the next class today and
    # pause the program till then regardless if its the first class or not
    if code is None:
        next_class_code = None
        next_class_time = None

        while next_class_code is None:
            for j in range(row + 1, eod_row):
                next_class_time = j
                next_class_code = ws[column + str(j)].value
                if next_class_code is not None:
                    break
            if next_class_code is None:
                print(f">No more classes scheduled for today {today.date()}")
                return None
        code = next_class_code
        row = next_class_time
        next_class_time = str(datetime.date(today)) + " " + str(ws["A" + str(next_class_time)].value)
        next_class_time = datetime.strptime(next_class_time, "%Y-%m-%d %H:%M:%S")
        print(f">Next class:: {next_class_code} @ {next_class_time}")
        pause_time = next_class_time - today
        print(f">Sleeping for {pause_time}")
        sleep(pause_time.total_seconds())
        print(">Sleep complete")
        print(">Continuing")
    # Start of 2 hr class
    if code == ws[column + str(row + 1)].value:
        entry_time = str(datetime.date(today)) + " " + str(ws["A" + str(row)].value)
        entry_time = datetime.strptime(entry_time, "%Y-%m-%d %H:%M:%S")
        exit_time = entry_time + timedelta(hours=2)
    # Joining mid way, a two hour class
    elif code == ws[column + str(row - 1)].value:
        entry_time = str(datetime.date(today)) + " " + str(ws["A" + str(row-1)].value)
        entry_time = datetime.strptime(entry_time, "%Y-%m-%d %H:%M:%S")
        exit_time = entry_time + timedelta(hours=2)
    # Normal one hour class
    else:
        entry_time = str(datetime.date(today)) + " " + str(ws["A" + str(row)].value)
        entry_time = datetime.strptime(entry_time, "%Y-%m-%d %H:%M:%S")
        exit_time = entry_time + timedelta(hours=1)

    return [code, entry_time, exit_time]


def input_menu():

    try:
        x = pytimedinput.timedInput("Please enter any of the above options:: \n", timeOut=30)
        return x
    except KeyboardInterrupt:
        pass


def menu(class_):
    global auto_exit_after
    threads = class_.create_threads()
    start_threads(threads)
    now = datetime.today()
    until_exit = class_.exit_time - now + timedelta(minutes=auto_exit_after)
    print(f">[{now.time()}] Auto-exit in {until_exit}")
    print("""  
    [Exit]: Exit current class.
    [Rejoin]: Rejoin current class.
    [Refresh]: Exit and rejoin the current class.
    [Type]: To mark attendance.
    [Ctrl + C]: Quit BOT.
                """)
    while True:

        if class_.exit_time + timedelta(minutes=auto_exit_after) < datetime.now() and class_.exited is False:
            print(">Killing class object")
            class_.exited = True
            join_threads(threads)
            class_.exit_meeting()
            del class_
            break

        elif class_.exit_time < datetime.now() and class_.exited is True:
            print(">Killing class object")
            join_threads(threads)
            del class_
            break

        input_ = input_menu()
        input_bool = input_[1]
        user_input = input_[0]

        if input_bool is True:
            pass

        elif input_bool is False:

            user_input.lower()

            if user_input == "exit":
                if class_.exited is False:
                    class_.exited = True
                    join_threads(threads)
                    class_.chat_type("Mihir Shah 2K20/A17/75 Present")
                    class_.exit_meeting()
                    if class_.exit_time < datetime.now():
                        print(">Killing class object")
                        class_.exited = True
                        join_threads(threads)
                        class_.exit_meeting()
                        del class_
                        break

                elif class_.exited is True:
                    print(">You have already exited this class.")

            elif user_input == "rejoin":
                if class_.exited is True:
                    class_.exited = False
                    class_.join_meet()
                    threads.clear()
                    threads = class_.create_threads()
                    start_threads(threads)

                elif class_.exited is False:
                    print(">You are already in the class")
                    print(">Enter [refresh] command to exit and join again.")

            elif user_input == "refresh":
                if class_.exited is False:
                    print(">Exiting class")
                    class_.exited = True
                    join_threads(threads)
                    class_.exit_meeting()
                    print(">Joining meet again")
                    class_.join_meet()
                    threads.clear()
                    threads = class_.create_threads()
                    start_threads(threads)
                else:
                    print(">Class already exited try [Join] command")

            elif user_input == 'type':
                if class_.exited is False:
                    class_.chat_type("Mihir Shah 2K20/A17/75 Present")
                else:
                    print(">You are not in the class, Rejoin!")

    print(">Class over!")


def main():
    main_driver = open_chrome("Profile 1")
    while True:
        call = time_check()
        if call is None:
            print(">EOD")
            break
        new_class = Schedule(call[0], call[1], call[2], main_driver)
        menu(new_class)

    main_driver.quit()


if __name__ == "__main__":
    main()
